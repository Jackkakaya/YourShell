...
pub(crate) fn parse<I, T>(args: I) -> ParseResult<HiArgs>
where
    I: IntoIterator<Item = T>,
    T: Into<OsString>,
{
    parse_low(args).and_then(|low| match HiArgs::from_low_args(low) {
...
fn parse_low<I, T>(args: I) -> ParseResult<LowArgs>
where
    I: IntoIterator<Item = T>,
    T: Into<OsString>,
{
    crate::logger::Logger::init();
    let cli_args: Vec<OsString> = args.into_iter().map(Into::into).collect();

    let parser = Parser::new();
    let mut low = LowArgs::default();
    if let Err(err) = parser.parse(cli_args.iter().cloned(), &mut low) {
