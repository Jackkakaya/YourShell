 ...
 */
static int usage(int code, int keep_it_short) {
  FILE *f = stderr;
...
#define jq_exit_with_status(r)  return abs(r)
#define jq_exit(r)              return ( (r) > 0 ? (r) : 0 )
...
int umain(int argc, char* argv[]) {
#else /*}*/
int ys_jq_main(int argc, char* argv[]) {
#endif
  jq_state *jq = NULL;
  ...
  if (!program) { ret = usage(2, 1); goto out; }
...
out:
  ...
  // PATCHED (YourShell): upstream closes stdout here because the process is
  // about to end anyway, and closing is the only way to see a deferred write
  // error. We run inside a long-lived app process whose fd 1 is the caller's
  // pipe — closing it would break every command that runs after this one, for
  // the life of the app. Flushing surfaces the same deferred errors without
  // destroying the descriptor.
  if (fflush(stdout)!=0 || badwrite) {
    fprintf(stderr,"jq: error: writing output failed: %s\n", strerror(errno));
    ret = JQ_ERROR_SYSTEM;
  }
  // The FILE* is process-global and outlives this command, so a sticky error
  // flag would make every later jq report a write failure it did not have.
  clearerr(stdout);
  clearerr(stdin);
