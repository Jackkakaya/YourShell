...
main(int argc, char *argv[])
{
	...
	/* The frontend is embedded and may be invoked repeatedly in one process.
	 * Reset getopt/state globals that a normal process would get for free. */
	a_opt = C_opt = c_opt = f_opt = j_opt = L_opt = n_opt = o_opt = p_opt = 0;
	q_opt = t_opt = u_opt = v_opt = Z1_opt = version_opt = 0;
	unzip_debug = zipinfo_mode = tty = unzip_exclude_mode = 0;
	d_arg = O_arg = P_arg = NULL;
	y_str = "";
	noeol = 0;
	passphrase_buf = NULL;
	bsdunzip_optind = 1;
