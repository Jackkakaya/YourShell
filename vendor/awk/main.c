...
/* iOS in-process embedding: one-true-awk normally calls exit() on fatal and
 * usage errors. In our long-lived host process that would kill the whole app,
 * so FATAL (lib.c) and the usage paths below longjmp back to ys_awk_main when
 * this guard is active instead of exiting. See core/src/awk_adapter.rs. */
jmp_buf ys_awk_jmp;
int ys_awk_jmp_active = 0;
...
int ys_awk_main(int argc, char *argv[])
{
	const char *fs = NULL;
	char *fn, *vn;

	/* Reset process-global state so repeated in-process invocations behave
	 * like fresh awk processes. main.c file statics: */
	pfile = NULL;
	maxpfile = 0;
	npfile = 0;
	curpfile = 0;
	...
	/* lib.c / run.c input+stream state (firsttime, argno, infile, stdio EOF
	 * flags on the shared stdin/stdout FILE*). */
	ys_awk_reset_io();

	/* Guard: FATAL and usage-error paths longjmp here instead of exit(). */
	ys_awk_jmp_active = 1;
	if (setjmp(ys_awk_jmp) != 0) {
		ys_awk_jmp_active = 0;
		fflush(stdout);
		return 2;
	}
