...
impl CommandBuilder<'_> {
    ...
    fn execute(self) -> Result<CommandResult, CommandExecutionError> {
        ...
        // YourShell patch: build the argv and hand it to the pluggable runner
        // (see `crate::exec_hook`) instead of `std::process::Command`. iOS
        // forbids fork/exec, so on that platform the embedder runs the command
        // in an in-process subshell; with no hook installed this is still
        // `std::process::Command`, unchanged.
        let mut argv: Vec<OsString> = vec![entry_point.to_os_string()];
        ...
        if let Some(replace_str) = &self.options.replace {
            ...
            argv.extend(initial_args.iter().map(|arg| {
        ...
        match &self.options.action {
            ExecAction::Command(_) => {
                let env: Vec<(OsString, OsString)> =
                    self.options.env.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
                let code = crate::exec_hook::run(&argv, &env, None, self.options.close_stdin);
                if code == 0 {
                    ...
                    // The runner reports a signal death as the negated signal
                    // number, mirroring `ExitStatus::signal()`.
                    Err(CommandExecutionError::Killed { signal: -code })
                } else {
