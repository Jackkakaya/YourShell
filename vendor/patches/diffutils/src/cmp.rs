...
/// YourShell patch: marker returned instead of exiting on `--help`.
pub const HELP_REQUESTED: &str = "\u{0}yourshell-help";
...
pub fn parse_params<I: Iterator<Item = OsString>>(mut opts: Peekable<I>) -> Result<Params, String> {
    ...
    while let Some(param) = opts.next() {
        ...
        if param == "--help" {
            ...
            // YourShell patch: was `std::process::exit(0)`. `cmp --help` must not
            // terminate the host process, which is shared with the UI and every
            // other command. The sentinel is recognised by `main` below.
...
//     and 2 means trouble.
// YourShell patch: signature widened off `ArgsOs`, which can only come from
// `std::env::args_os()` and cannot be built from a shell's argv.
...
pub fn main<I: Iterator<Item = OsString>>(opts: Peekable<I>) -> i32 {
    let params = match parse_params(opts) {
        Ok(param) => param,
        Err(e) if e == HELP_REQUESTED => return 0,
        Err(e) => {
            eprintln!("{e}");
            return 2;
        }
