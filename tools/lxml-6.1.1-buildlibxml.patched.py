...
def build_libs(
    ...
    # Cross-compile support: autotools libs (iconv/libxml2/libxslt) need an
    # explicit --host so configure skips run-tests it can't execute on the build
    # machine. Injected via env so the same source builds for iOS.
